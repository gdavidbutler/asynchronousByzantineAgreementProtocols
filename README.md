...
The paper's proofs depend on three assumptions about the message system (Section 2):
...
Without BPR, an application using this library would need its own bookkeeping to track per-instance destinations, per-process evidence of receipt, and a cursor-driven retransmit retry -- the standard "deployment-layer reliability" pattern. Three properties make BPR a better fit:
...
2. **The retry is event-driven, not wall-clock-driven.** The application's retry tick is the only event; no `retryNs` floor, no per-instance evidence tracking, no destination masks.
...
while (!terminate) {
  ...
  while (network_recv(&msg)) {
    ...
    if (msg.type == BRACHA87_READY && !msg.answered)
      /* The ABSENCE of BKR94ACS_ANSWERED is the sender saying it does
       * not hold OUR accept -- it would have suppressed us otherwise.
       * A caller that parks quiescent instances re-enters them here. */
      (msg.cls == BKR94ACS_CLS_ACAST)
        ? bkr94acsAcastWants(a, ...)
        : bkr94acsBaWants(a, ...);
...
./example_bracha87Fig1 -b 2 4 1 hello           # Byzantine initiator equivocates (split=2 and split=3 stall; split=1 and split=4 converge)
...
The same policy serves the bare Fig 1 surface (`example/bracha87Fig1.c`): progress is a `bracha87Fig1Input` that returns actions (the same dedup keeps the counter retransmission-stable) or an ACCEPT; the sweep is one full `bracha87Fig1RetryStep` pass over the caller's array (`bracha87Fig1SentCount` calls); and Fig 1 has no EXHAUSTED -- reliable broadcast has no phase ceiling, so abandonment and quiescence are the only exits that layer has. `example/bracha87Fig1.c` takes the second: it wires both READY annotations -- ACCEPTED (`struct bracha87Fig1Act.accepted` on egress, `bracha87Fig1ProcessAccepted` on ingress) and ANSWERED (`.answer` on egress, `bracha87Fig1ProcessWants` on ingress) -- which together are what make the retire reachable there at all, and it re-enters a quiescent instance in the rotation whenever a want arrives.
...
**Byzantine-silent processes.** Up to t processes that never send are not a failure mode; the run completes with their A-Casts excluded. Their cost is the retry tail: every gate that needs all n -- all-echoed, the READY quiescence gate -- can never close, so READY retries toward the silent processes continue until abandonment retires them. That tail is correct, not waste: a silent process is indistinguishable from a laggard that still needs the READY (previous scenario), and per-process suppression has already dropped every process that crossed. The conservative default -- retry until the application abandons -- is the right one.
...
**After COMPLETE.** Success is not a stop: other processes may still be below their thresholds, and post-decide continuation owes them the READY traffic that will carry them (the partition scenario, seen from the other side). The two READY annotations give the tail a true end: once every process has announced its accept (`ACCEPTED`) and holds this one's (`ANSWERED`), Retry's full sweep returns 0 -- quiescence -- and nothing more is owed to anyone, so stopping then needs no further patience. Quiescence is REACHABLE under fair loss rather than guaranteed: a lost answer leaves the wanter's mask unchanged, so its next request re-arms the reply and fair loss delivers one eventually -- but that recovery has a caller obligation attached. A process that drops an instance from its retry rotation on the 0 return must put it back when a want arrives, since only a tick can answer; a caller that leaves for good forfeits the recovery on the first dropped answer. What no annotation can reach is the honest residue: a process that abandons early, or one that never announces at all, holds every other process's gate open forever. So the barren-sweep gate remains the backstop: a `COMPLETE` process keeps draining and ticking, and leaves through the same gate -- the success flavor of the one policy.
...
13. **READY's only sound retire is *remote*, and it takes TWO facts -- never *local* accept (the per-process refinement of Note 10).** Per-process suppression and the READY quiescence gate both read PROCESSES' accepts, announced via the `BKR94ACS_ACCEPTED` wire bit and recorded in `acFrom` -- not the local `ACCEPTED` flag that Note 10 forbids. But "q has accepted" and "q holds MY accept" are different facts, and only the READY being retried carries the second. Suppressing on the first alone silences the announcement q waits for, so q's own gate stands one bit short for good -- reachable with no loss at all, by nothing more than a cursor phase offset. The second fact needs its own wire bit: `BKR94ACS_ANSWERED` on a READY says the sender has recorded *this recipient's* accept, so its ABSENCE is a request, routed to `bkr94acs{Acast,Ba}Wants`, which un-suppresses that sender for one egress. Answers never arm, which is what keeps the exchange from ping-ponging -- without the discriminator an answer is byte-identical to a request and the honest run never falls silent. Per-process: skip READY to a process once *it* has accepted AND holds ours. Whole-action: stop only when that holds for all `n`. Two Byzantine-safety facts are load-bearing: a forged `ACCEPTED` marks only its own sender, so it retires our retry to that liar but can never strand a correct laggard (whose bit is set solely by its own true accept); and reaching the all-`n` gate requires every *correct* process's true accept regardless of what the <=t Byzantine processes claim. Do **not** add a `>=2t+1 accepted -> stop` threshold shortcut -- Byzantine forgeries could trip it while a correct process is still below 2t+1 readys. Regression checks: `testFig1SkipAccept`, `testBprSkipAccept`, and `runWithRetry` drop-convergence with suppression active.
