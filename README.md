...
| 11 | Bracha Fig 4, step 3; Section 4 | "Go to round 1 of phase i+1" after every case; "For notational convenience, the protocol in Fig. 4 does not terminate once a decision is made. However, this can be easily accomplished." | A decided process keeps broadcasting and runs every later phase as the figure writes it (the figure has no decided state; Lemma 9 holds its value at the decision); DECIDE is returned once; no halting is built | paper for the figure, which never halts; construction for building no halting, where the paper says it can be accomplished and constructs none | Note 1 (and 2, 9); `bracha87Fig4Round` in `bracha87.h` | `testFig4PostDecide`, `testFig4PostDecideUngated`, `testPostDecideMultiPhase`, `testPostDecideContinuation`; `test_bracha87_blackbox` "figure-unchanged arm" |
| 12 | Bracha Fig 4, phases | Unbounded | `maxPhases` 1..85, refused outside; `BRACHA87_EXHAUSTED` when spent, no substitute decision, COMPLETE unreachable | construction | Note 12; *The phase budget*; `bracha87Fig4Sz` | `testExhausted`, `testExhaustedAmongDecided`, `testFig4MaxPhasesRefused`; Section D |
| 13 | Bracha Fig 4, step 3 (iii) | "coin_toss (0 or 1 with probability 1/2)" | `bracha87CoinFn(closure, instance, phase)`, an oracle the caller supplies; `bkr94acs` names `instance` with the BA's process index | construction (the callback's shape and the coin's name; its value is the caller's) | *Coin Choice*; `bracha87CoinFn` in `bracha87.h` | `testFig4Step3Boundary` (dc == t coins, not adopts), `testFig4MultiPhase`; no check grades a coin; `test/measure_phases.c` measures the local one, as an instrument |
...
| 21 | Bracha Fig 4, step 1 of phase i+1 | "Broadcast(p, 3i+1, value_p)" as the step opens | Under a local coin a deployment may choose, at `bkr94acsInit`, to hold the broadcast that carries a step-3 outcome -- the coin, when case (iii) fired -- at the turn, and the caller releases it (`bkr94acsBaReveal`, `bkr94acsBaHeld`); to every receiver a held-then-released INITIAL is a delayed message, which Section 1's model admits, and Lemma 8 carries under the premise that every hold is released | construction | `BPR.md` (*The Sweep-Side Decisions*, the hold); `bkr94acsBaReveal` in `bkr94acs.h`; `bracha87Fig1Hold` in `bracha87.h`; *Delivery time* | `testFig1Hold`, `testReveal`, `testRevealClamp`; mutants M75-M83 |
...
```bash
./example_bkr94acs 4 1 joe sam sally tim        # 4/4 agreed, every process quiescent at tick 297
...
./example_bkr94acs -l -r 2 4 1 joe sam sally tim  # -l: a local coin and the hold; -r: each reveal two sweeps late (quiescent at tick 299)
...
Delayed process 3 (patience 0 sweeps, 296 ticks): EXCLUDED -- zero patience shut the door
...
...
All processes agree on subset: ok
Delayed process 3 (patience 1 sweeps, 298 ticks): INCLUDED -- patience let step 1 win
...
Nothing beyond the four obligations above is required of the transport. A caller that chooses the hold (below) owes one call more, whatever its transport: the reveal. A complete deployment is that transport, a coin source, and an abandonment policy. The library is protocol-only and supplies none of them.

### The coin, across the stack

The coin is where the papers' randomization meets a deployment, and what bears on it is spread over every document here, each place carrying its own argument. This is the map.

- **The papers.** `Bracha87.txt`: Fig 4 step 3 case (iii), the figure's own local coin; Theorem 2's proof, which bounds from below a phase's chance of converging; Theorem 3, how the phase count scales with n and t (its constant case is Ben-Or's, `BenOr83.txt`); Section 7, Rabin's dealt global coin. `BKR94ACS.txt`: the contrast between secret sharing's termination and agreement's, which this tree reads as the price of generating a coin inside the run (`bracha87.h` at `bracha87CoinFn`; *Why these papers*). Why randomize at all is FLP (`FLP82.txt`).
- **The headers.** The coin is a callback, `bracha87CoinFn`, bound at `bracha87Fig4Init` or `bkr94acsInit`; `bracha87.h` says why it is an oracle, and why its name is (instance, phase) -- the name a global coin must agree on, which `bkr94acs` sets per BA. `bkr94acsInit` fixes three choices together: the coin, the phase budget `maxPhases`, and `hold`. The hold is one mechanism with a pair of names at each layer, each pair named for what it does there: `bracha87Fig1Hold` / `bracha87Fig1Release` hold and release any initiator's broadcast; `bkr94acsBaReveal` releases the one that carries a coin and `bkr94acsBaHeld` counts those still held -- and why only a local coin wants it is at `bkr94acsBaReveal`. A fourth choice is not fixed at Init: the round turn's patience, spent per sweep (`bkr94acsTurn`; *Abandonment*).
- **This file.** *Delivery time* (what the hold must beat, and what a release discipline buys), *Coin Choice* (the three coins), *The phase budget* (what `maxPhases` must cover), *Sizing instead of sharing* (choosing N so the phase count stops growing with it), *Abandonment* (the turn's patience), *Why these papers* (no bundled coin), *Test coverage* (what the battery owes the coin, and what it does not); Departures rows 13 and 21; Implementation Note 9 (why a decided process never reaches the coin).
- **`BPR.md`.** The hold as a retry state that is not a retire (*Retirement*), as the third seam (*The Sweep-Side Decisions*), and as a registry entry with its teeth (*The Scoped-Claim Registry*).
- **The audit chain.** The "held" input of `bracha87Fig1.dtc`; `bracha87Fig4.dtc`, where the table selects the coin output and the C invokes it.
- **Measured.** `make measure` (`test/measure_phases.c`: a local coin's phase budget, adversary by adversary) and `make drift` (`test/measure_drift.c`: whether a release discipline keeps every coin unexposed).
- **Tested.** The coin runs in `testFig4MultiPhase` (scripted), `test_bkr94acs_blackbox` Section R (a per-process pseudo-random coin, split, with the hold through the phase) and the schedule explorer's adversary configs (the deterministic coin; `test_schedules.c` says how its counts are read); the window a toss opens at a validating peer in `testFig4AdoptWindow`; the hold's mechanism in `testFig1Hold`, `testReveal`, `testRevealClamp` and mutants M75-M83; Note 9's two rules in M62 and M63. The examples never toss a coin; `example_bkr94acs -l` shows the hold's calls.

### Delivery time, what a local coin's budget needs

The four obligations of *Message System* bound nothing in time, which is the papers' model, and in it the adversary schedules: it chooses which n-t validated messages each turn computes over, and the t faulty vote as it likes. That it also knows every coin already tossed is a reading -- Bracha states his scheduler only as choosing which n-t messages each process receives; Ben-Or's schedule "may depend on the current configuration as well as on all the past history" -- and it is the reading the harness plays. Against that adversary a local coin's per-phase convergence is `2^-(n-t)` at n = 3t+1, which is the bound Theorem 2's proof gives (rho >= 2^-(n-t)) and the adversary the harness plays attains it there, so that bound cannot be improved; it rises only as N grows past 3t+1, and at N = t*t it is on its way to Theorem 3's constant: one phase in 315 at 16/4 (N/t, here 16 processes and t = 4), one in 76 at 100/10, one in 61 at 256/16 (the harness's table), still rising toward the normal tail at 2c, about one in 44 in the limit. At that sizing, N = max(t*t, 3t+1), no budget inside 1..85 brings the chance of exhausting it below 1e-6 for any t, the floor 4/1 included; a larger N does, and the harness's table names the N above which every larger N fits, 10 at t = 1 for 1e-6 and 16 for 1e-9 (*The phase budget*). That is the whole of what the four obligations support, and a deployment that declares nothing more sizes from it.

What the adversary needs is time the model does not have. Its play is to read one correct process's coin off the step-1 broadcast that carries it and, before another correct process's step-3 turn of the same phase fires, hand that process t+1 d-messages (case (ii), its own among them) or fewer (a toss), one process at a time. A deployment that can bound when every correct step-3 turn of a phase has fired can deny it that: hold the step-1 broadcast that carries the coin until then. The library carries the hold as a choice made at `bkr94acsInit` -- with it `bkr94acsTurn` withholds the phase-opening INITIAL, and `bkr94acsBaReveal` releases it on the caller's word (the contract, which coin wants it, and why a hold is safe under any release policy, are at `bkr94acsBaReveal` in `bkr94acs.h`; the Fig 1 half is `bracha87Fig1Hold`). Only a local coin has a use for it: no step-1 broadcast tells a faulty process anything about a global coin hosted behind `bracha87CoinFn` that its own closure cannot already name, so a global coin is deployed without the hold. A caller that holds and declares no bound reveals at once, which is the papers' model.

What the release discipline must bound is not a latency but a spread: the interval between the first and last correct step-3 turn of a phase. A bound on one hop does not bound it, because enabling is not a common event -- the faulty's t votes count toward n-t, so a process they deliver to enables on the (n-2t)th correct arrival and a process they withhold from needs every correct message, and with two or more processes so treated the gap grows round over round (measured below). Two disciplines do bound it, and both are stronger than a delivery bound: a local metronome, each process firing its turns no sooner than a fixed interval W after its own previous turn and revealing a fixed interval H after its step-3 turn; or synchronized slots, a common clock. `test/measure_drift.c` (`make drift`) is a timing model of the drift and of the metronome -- times only, no protocol, with Fig 1's three hops modeled as the order statistics its thresholds name over the correct arrivals, fewer when the faulty help a broadcast and more when they do not -- and it measures two rules under two delay laws, one hop bounded by one, and one hop in a hundred stretched to as much as five. The rule anchored on enabling drifts at every t of 2 or more, by one to three hops per phase (about 0.4 to 1.0 hop per round) over the configurations modeled with hops bounded, with the laggards exposed once the drift has outrun the hold (the exposure the model prints is the fraction of correct processes whose step-3 turn fires after another correct process has revealed its coin); at t = 1 it does not, since one laggard waits on nobody late. With hops bounded the metronome holds the spread when its period covers the hold plus an unhelped validation, about 1.7 to 2.6 hops in the model, and its exposure is zero only when the hold also covers the entry spread, which the split case -- some processes entering a BA by step 1 and the rest by step 2's fanout -- sets by the step-2 patience: with H at or above that spread and W at or above H plus three hops, exposure is zero at every configuration modeled. Under the stretched hop it is not: the spread ratchets, since nothing in a metronome brings back a laggard that slipped once, and at 7/2 with W = 8 and H = 5 exposure is back by the fourth phase (0.03 to 0.05) and at 0.14 to 0.18 by the eighth; wider slack (W = 12, H = 8) holds 7/2 within 0.01 over eight phases but not 10/3 (0.03 to 0.07 by the eighth), and the ratchet is still there. So a deployment's discipline must either bound its tail or re-synchronize, and the model says which of its own rows it sits on. What zero exposure buys is the band of width t the reduction leaves to the faulty's own votes rather than the scheduler's 2t -- and nothing less than zero buys it: arm K of `test/measure_phases.c` lets the adaptive force reach only the exposed processes, and at every t it runs from 2 to 7, exposing t of them already lands on the full adaptive figure, within 20% either way of arm I's printed closed form in the two runs printed, about what two runs of 200 trials differ by; at t = 1 exposing the one does not (its figure is about three times arm I's). Two qualifiers stay on the sentence: the vote band assumes the faulty cannot also steer which correct messages a sample holds, which at n = 3t+1 they can through Fig 1's accept threshold whenever a correct ready is lost (arm J), so it assumes a transport that loses none by the turn; and the model's hops are not the deployment's sweeps, so what H is in a deployment's own unit is its declared physics, not this file's. Under the metronome with zero exposure and that transport, the per-phase figure is arm H's, and its budget over the worst N at or above max(t*t, 3t+1) is the vote-band table `test/measure_phases.c` prints, 47 phases at 1e-6 and 69 at 1e-9 over every t up to 16. Section R of `test_bkr94acs_blackbox` is the liveness half red-proven: staggered releases through a coin phase agree and complete, and a hold never released blocks a phase only together with a crash that spends the rest of the t.
...
while (!terminate) {
  ...
  for (p = 0; p < N; ++p)
    for (;;) {
      n = bkr94acsTurn(a, p, acts);
      n += bkr94acsBaReveal(a, p, acts + n);   /* 0 under hold 0; else reveals at once */
      if (!n) break;
      for (k = 0; k < n; ++k) broadcast_action(acts[k]);
    }
...
|---|---|---|---|---|
| `bracha87Fig1.dtc` | `bracha87Fig1ToC.dtc` | `bracha87Fig1Rules.c` | `bracha87Fig1Input` and `bracha87Fig1Bpr` (one snippet, two `#include`s; the six paper rules and BPR's five as sub-tables, chained) | 14 |
...
| `test_bracha87` | Protocol white-box (bracha87) | Per-rule units, composed simulation with inline lemma/theorem checks, equivocation, post-decide continuation, BPR retirement invariants, both-sided thresholds; reads internal flags |
...
| `test_bkr94acs` | Protocol white-box (bkr94acs) | All-to-all simulation, the step-2 trigger and floor, post-decide continuation, BPR drop-convergence, the Byzantine-silent canary, EXHAUSTED handling, the forged INITIAL, the hold and its reveal under both choices, a reveal past the round space; reaches into internal layout |
| `test_bkr94acs_blackbox` | Protocol black-box (bkr94acs) | Sections A-R: Lemma 2 Parts A-D, Input dedup, Retry and quiescence under drop, EXHAUSTED, the equivocating A-Caster, step-2 and round-turn pacing, the *Abandonment* scenarios (Sections I-N), a BA's decision versus this process's input, annotation forgery, the paired payload's retire and hold, and (Section R) the coin's hold across processes through a coin phase -- no `.c` reads |
...
1. **Post-decide continuation.** Fig 4 says "Go to round 1 of phase i+1" after all three step-3 cases, so the figure never halts, and Theorem 2 is proved for that figure: Lemma 8's no-deadlock proof takes every correct process to have broadcast at the first blocked round. A process that stops sending at its decision is, to every other process, a faulty transmitter -- in the model there is no way to distinguish between a "slow" message and a message not sent (Section 1) -- and the paper's only word on halting is one unproved clause. So a decided process keeps broadcasting; `BRACHA87_DECIDE` is returned once, with `BRACHA87_BROADCAST` whenever a next round exists, alone on the last phase, so callers test the bit (the argument in full is at `bracha87Fig4Round`). Regression: `testFig4PostDecide`, `testPostDecideContinuation`.

2. **D_FLAG leak.** Case (i) is two assignments, `decision_p := value_p := v`, and only the first is once-only. An implementation that gates the whole case on "not yet decided" leaves the value where step 2 put it, `(d, v)`, and the flag rides into the next phase's step-1 broadcast -- which no correct process sends (every step-3 case writes a bare value) and VALID therefore rejects. The dispatch gates `decide v` alone; `adopt v` (the `value_p := v` half, shared with case (ii)) fires every phase. Regression: `testPostDecideMultiPhase`; mutant M42 gates the value half on the decided state against its leak check.
