...
| Replay action | Flag gate | Retires when | Notes |
|---|---|---|---|
| `BRACHA87_INITIAL_ALL` | `F1_ORIGIN` | `ACCEPTED`, or echo observed from all `n` peers | Two independent sound stops. All-echoed: INITIAL only induces echoes, so none left to induce. ACCEPTED: `t+1` correct readys now circulate, so amplification finishes with no INITIAL. The *weaker* "stop once we ECHOed locally" gate stays forbidden — at the n=3t+1 boundary the (n+t)/2+1 echo threshold equals the honest count, so a peer that missed the bootstrap can leave the cascade one echo short forever, and only the originator breaks it. Under ≤t byzantine-silent peers the all-echoed count can't reach `n`, so that path self-disables and ACCEPTED carries the retirement. |
| `BRACHA87_ECHO_ALL` | `F1_ECHOED` | `ACCEPTED` | Same amplification argument: past `t+1` correct readys no correct peer consumes an echo. Chained DTC rule (fires when Bracha is not firing send-echo and have-echoed is set), gated off at ACCEPTED in C. |
| `BRACHA87_READY_ALL` | `F1_RDSENT` | never (only application abandonment) | Continues post-accept. The "ACCEPTED → stop replaying READY" optimisation strands slow peers — an accepted peer still owes its READY to peers below `2t+1`, and replay is Bracha's only delivery mechanism under loss. |
