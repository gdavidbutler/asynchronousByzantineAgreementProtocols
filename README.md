...
**The coin is an oracle, not a protocol, and the callback's shape says so.** `bracha87CoinFn` returns a value -- always, immediately, with no way to fail, decline, or block. That is deliberate and it is load-bearing. Randomization is what buys probabilistic termination where FLP denies the deterministic kind, and the escape only holds if the randomness comes from *outside* the asynchronous system. A construction that must exchange messages before it can answer therefore cannot run *behind* this callback -- there is no point in the round for the exchange to happen. That is a statement about the interface, not about the construction: Bracha's Section 7 does exactly such an exchange, with the processes accessing "the global coin toss by exchanging portions of it" and reaching an expected two phases. Note where the randomness comes from there -- a dealer, before the run. A coin that instead generates fresh randomness by agreement inside the run gives up what the local coin keeps: BKR94 records that its secret-sharing route carries "an exponentially small but non zero probability of not terminating", against "the asynchronous Byzantine Agreement problem where the randomized protocol terminates with probability 1." It would be circular here besides: the exchange needs a reliable broadcast, and the one at hand is Fig 1, so this library's agreement would rest on a coin resting on this library. Supply randomness the deployment already has, and have its per-phase values in the closure before the call.
...
Bracha defers his Theorem 3 to Ben-Or 1983 ("Another Advantage of Free Choice: Completely Asynchronous Agreement Protocols (Extended Abstract)," PODC '83), where it reads: "If t = O(sqrt(N)) then the expected number of rounds to reach agreement in protocols A and B is constant, (i.e. does not depend on N)." (The scan reads "Ift" for "If t" and a radical for `sqrt(N)`; this repo is ASCII.) Bracha restates it as `t = c*sqrt(n)` giving a constant independent of `n`, though exponential in `c`; at `t = c*n` it is exponential in `n`. Theorem 3 is his only phase-count result -- Theorem 2 is the resilience theorem, not a bound on phases. Bracha's proof citation points to that extended abstract, which states Theorem 3 without proving it; this library uses the published result rather than re-deriving it.
...
- **`test/test_bracha87.c`** and **`test/test_bkr94acs.c`** are the integration-test corpus, including the regression checks named in Implementation Notes #9-#12 and #16-#17.
