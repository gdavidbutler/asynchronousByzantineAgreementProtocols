...
#
# Output is a bare snippet: labels, gotos, tests, assignments, preceded by
# a two-line provenance comment carrying the .psu's D row -- dtc's depth
# for the dispatch.  The snippets are tracked and the .psu is not, so
# without that line the depth is unrecoverable from the committed tree
# without re-running the search, and nothing marks the file as generated.
# Caller wraps the snippet with declarations of the boundary locals and
# any pre/post side-effect code.
...
/^[LJTR],/ { body[++nbody] = $0; src = FILENAME }
/^D,/ {
  parse_csv($0, f)
  depth = f[2]
}
...
END {
  print "/* Generated by psu.awk from " src (depth == "" ? "" : " -- dispatch depth " depth) ". */"
  print "/* Edit the .dtc and regenerate; changes here are overwritten. */"
  for (i = 1; i <= nbody; i++) {
    line = body[i]
    parse_csv(line, f)
    if (line ~ /^L,/) {
